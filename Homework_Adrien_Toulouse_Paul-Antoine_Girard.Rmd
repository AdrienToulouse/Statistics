...
So, the range of the confidence interval found with the estimator $\hat\theta_{2}$ is larger and is therefore less precise.
...
str(USJudgeRatings)
```
The data is stored in a dataframe and we can observe that all the variables are numeric.
\newline
...
By looking at the medians and means, we know that we can have information on the distribution of the variables. In fact, means are really sensible of outliers when medians aren't. However, in our case, each variable admits a median that is close to the mean. For example, the median for the INTG variable is 8.021 and its median is equal to 8.100. So we don't have any information on possible outliers.  
...
Therefore, we need to see if there a link between the other variables and the variable RTEN.
...
For example, for the CONT variable, we can see a bell curve with its highest point near the mean of the sample. So we can assume that this variable follow a gaussian distribution. However, we also see the two outliers on the histogram and which modify a bit the symmetry of the distribution.  
The INTG variable seems to also follow a gaussian distribution, but it's less obvious. The bell curve is even less symetric. However, the other two variables don't seem to follow any usual distribution and it's hard to find a real particularity.
\newline
...
We focuse on four of the variables and we look at their empirical cumulative distribution functions.  
...
\newline
Finally, let's compare the quantiles of our variables to quantiles of a gaussian distribution to see if we can really associate these variables to gaussian distributions.
\newline
...
This analysis can be explain by the outliers we found at the beginning. In fact, we found outliers with upper value for the CONT variable and outliers with lower values for the RTEN variable. So that explain why we don't find a gaussian distribution for these extreme values.  
\newline

### Let's analyze the correlation between the variables

First, we can take a look at the covariance matrix, and the standard deviation of each variable.
```{r cov}
...
Let's measure the correlations between the 12 first variables to see if there is a link between them.
...
Obviously, we can see that there is a very strong correlation between eleven of the twelve variables. In fact, the variable CONT is the only one which isn't correlated with the other variables. The number of contacts of a judge with lawyers doesn't seem to explain the ratings received by the judge.
...
This correlation suggests us that we can create a linear model to predict some of the variables with others.  

Following this trend, we will take a look at the scatter plots between the different variables.

```{r pairs}
pairs(USJudgeRatings, gap=1/5, lower.panel = panel.smooth, upper.panel = NULL, row1attop=FALSE)
```

These plots show us what we suggest before. In fact, there is a linear correlation between eleven of the twelve variables (all the variables except CONT). If the subject would have been to predict the values of one of these variables, we could have used a linear regression model.
\newline
...
We calculate the Skewness of four variables. Skewness is an indicator that measures the assymetry of a distribution. In our case, we always have a negative indicator. That means that the tail is on the left side of the distributions.
\newline
...
However, we have seen that the variable PREP has a very lower outlier but its kurtosis is near 0. On the other side, we saw that the variable DECI doesn't have any outlier but its kurtosis is negative and near -0.5.  
But all these Kurtosis values are ranged between -1 and 1, so their values aren't extreme and that don't show any extreme particularity.
\newline

## Application of statiscal methods

in order to apply some statistic methods, we will construct a confidence interval for the mean of the RTEN variable. In fact, we assumed during this report that this variable follow a gaussian distribution. So, it will be interesting to estimate its mean without knowing its variance.  

We assume that we have n = 43 observations of a gaussian distribution:  $X_k \sim \mathcal{N}(\mu, \sigma^2)$, $\forall k \in {1, ..., n}$ such that  $\mu \in \mathbb{R}, \ \sigma \in \mathbb{R}_+^*$.  

Our statiscal model is therefore:
...
Since, we don't know the variance but we assumed that the observations are gaussians, we will use the fact that $S^2 = \frac{1}{n-1} \sum_{i=1}^{n} (X_i - \overline{X_n})^2$ is an unbiaised estimator of $\sigma^2$.  

We know that a confidence interval of level $\alpha$ for $\mu$ is:
...
print(CI)
```
So ,we find under our assumptions that the RTEN variable follows a gaussian distribution of mean $\mu$, with a confidence interval of level $\alpha$ for $\mu$ equal to:
$$[7.259487, 7.945164]$$

Now, if we don't assume that the RTEN variable follows a gaussian distribution. We can also construct a confidence interval for $\mu$ using Slutsky's theorem (which is possible because we have more than 30 observations).  
In fact, we assume that our observations are independent from a distribution, which has a mean equal to $\mu$ and a variance equal to $\sigma^2$.  
...
So , if we don't assume that RTEN follows a gaussian distribution, we find a confidence interval of level $\alpha$ for $\mu$ equal to:
$$[7.273254, 7.931397]$$
This asymptotic confidence interval has a smaller size than the previous one and is therefore more precise.
