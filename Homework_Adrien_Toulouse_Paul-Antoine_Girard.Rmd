...
Then, we can conclude that our estimator $\hat\theta_{MLE}$ reach the Cramer-Rao bound and is therefore efficient.
...
and applying the law of large numbers gives us: $$(\overline{X_n} - \theta)\overset{p} {\to} 0$$
...
So, by stability of the convergence in probability, we have that $\hat\theta_{2} \overset{p} {\to} \theta$ and by continuous mapping, $$\sqrt{2\hat\theta_{2}^2 + \hat\theta_{2}} \overset{p} {\to} \sqrt{2\theta^2 + \theta}$$
...
and we can use it as pivotal quantity to find a confidence interval for $\theta$.
...
$$
This asymptotic confidence interval has a range equal to $2 \cdot z_{1-\alpha/2} \frac{\sqrt{2\hat\theta_{2}^2 + \hat\theta_{2}}} {\sqrt{n}}$.  
The asymptotic confidence interval found at the question 7 has a range equal to $2 \cdot z_{1-\alpha/2} \frac{\sqrt{\hat\theta_{MLE}}} {\sqrt{n}}$.  
So, the range of the confidence interval found with the estimator $\hat\theta_{2}$ is larger and is less precise.
...
These four histograms tell us that the variables aren't symmetric. However, they all seem to have a bell curve as density.
\newline
...
Finally, these four variables seem to have a symmetric law if we don't take a deeper look on outliers. In fact, they all seem to have extreme lower values but there densities are bell curves with the highest point of probability near there mean. However, these histograms aren't precise. Maybe, we could gain in accuracy with a larger dataset.  
...
The QQ-plots are really interesting to analyse for these two variables. In fact, the qq-plot for the RTEN variable shows us until -1 the variable doesn't seem to follow a gaussian distribution but from -1 to 2, the quartile of the variable are really close to the quantiles of a gaussian distribution of a mean approximately equal to 7.4.  
...
Thys analysis can be explain by the outliers we found at the beginning. In fact, we found outliers with upper value for the CONT variable and outliers with lower values for the RTEN variable. So that explain why we don't find a gaussian distribution for these extreme values.  
\newline
...
These plots show us what we suggest before. In fact, there is a linear correlation between eleven of the twelve variables (all the variables except CONT). if the subject would have been to predict the values of one of these variables, we could in fact have use a linear regression model.  
\newline
...
We calculate the Skewness of four variables. Skewness is an indicator that measure the assymetry of a distribution. In our case, we always have a negative indicator. That means that the tail is on the left side of the distributions.
\newline
...
Kurtosis is an indicator that measure the propension of a distribution to produce outliers.  
...
find the paratemer of distribution of rten
